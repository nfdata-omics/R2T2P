# Look for the pattern "06/11/24" to see the 3 changes I made on that day
# Look for the pattern "15/01/25" to see the 4 changes I made on that day
# Look for the pattern "17/01/25" to see the only change I made on that day
# Look for the pattern "24/01/25" to see the 4 changes I made on that day
# Look for the pattern "27/01/25" to see the only change I made on that day
# Look for the pattern "16/04/25" to see the 2 changes I made on that day
# Look for the pattern "18/04/25" to see the changes I made on that day
# Look for the pattern "28/04/25" to see the only change I made on that day

#!/usr/bin/Rscript
# I commented the following line on 06/11/24
# .libPaths(c(.libPaths(),"/home/roberto.albanese/R/x86_64-pc-linux-gnu-library/4.1/"))

# I added quotes around the name "RiboseQC" in the following line on 06/11/24
suppressPackageStartupMessages(library("RiboseQC"))

#takes standard and de-novo GTFs
args <- commandArgs(trailingOnly = TRUE)
#args<-c("Homo_sapiens.GRCh38.82.gtf","HEK293_RF.gtf","cuffcmp.combined.gtf","merged_str_82_new")
#args<-c("Chlorocebus_sabaeus.ChlSab1.1.99_mod.gtf_Rannot","chloro_compare.annotated_ok.gtf")

# I replaced the following line with the next one on 15/01/25
# source("/group/calviello/new_riboseqc_all_HT-copy.R")
# I replaced the following line with the next one on 17/01/25
# source("/group/calviello/new_riboseqc_all_HT.R")
# I replaced the following line with the next one on 16/04/25
# source("/group/calviello/new_riboseqc_all_HT-copy.R")

#load_annotation(args[1])
...
trann <- unique(mcols(import.gff2(args[2], colnames = c("gene_id",
...
code_annot<-cbind(c("=","c","k","r","u","x","s","e","i","j","o","p","y","m","n","xi")
                  ,c("same","contained","extended","repeat","intergenic","antisense","antisense_spl","pre_mRNA","intronic","alt_isof","exon_overl","run_on","enclosing","retain_intr","retain_partintr","antisense_intr"))
...
# I replaced the following line with this if-else section on 24/01/25
# trann<-rbind(trann,str_ann[!str_ann$class_code%in%c("c", "="),colnames(trann)])
# I replaced the following if-else section with the next one on 16/04/25
# if (args[4] == "not_included") {
#   trann<-rbind(trann,str_ann[!str_ann$class_code%in%c("c", "="),colnames(trann)])
# } else if (args[4] == "included") {
#   trann<-rbind(trann,str_ann[,colnames(trann)])
# }
# Added "as.logical" in the two if statements on 18/04/25
# Replaced "args[4]" with "args[3]" on 18/04/25
...
# I replaced the following line with the next one on 15/01/25
# all$`source`="Annot_StringTie"
# I replaced the following line with the next one on 24/01/25
# all$`source`=paste0("Annot_", args[3])
# Replaced the following line with the next one on 18/04/25
# all$`source`=args[3]
...
#gtf_file<-paste(args[1],"_stringtie.gtf",sep="")
# I replaced the following line with the next one on 15/01/25
# gtf_file<-paste(args[2],"_tmp_stringtie.gtf",sep="")
# I replaced the following line with the next one on 24/01/25
# gtf_file<-paste0(args[2],"_tmp_", args[3], ".gtf")
# Replaced the following line with the next one on 18/04/25
# gtf_file<-paste0(args[3],".gtf")
# Replaced the following line with the next one on 28/04/25
# gtf_file<-paste0(args[2],"_merged_with_ref.gtf")
...
#The following line was added by Roberto on 25th May 2023
...
# I changed the name of the Rannot file in the following line on 06/11/24
# I replaced the following line with the next one on 15/01/25
# rannot_file<-paste(args[2],"_merged_with_gencodev38_Rannot",sep="")
# I replaced the following line with the next one on 24/01/25
# rannot_file<-paste0(args[2],"_merged_with_gencode_Rannot")
# I replaced the following line with the next one on 27/01/25
# rannot_file<-paste0(args[3],"_Rannot")
# I replaced the following line with the next one on 18/04/25
# rannot_file<-paste0(args[3],".gtf_Rannot")
# I replaced the following line with the next one on 28/04/25
# rannot_file<-paste0(gtf_file, "_Rannot")
